...
void main() {
  ...
  setUp(() {
    ...
    galleryRepository = GalleryRepository(provider: unsplashService);
  ...
  test('Should be able to get images', () async {
    expect(
        await galleryRepository.getImages(),
        List.generate(listOfImageMap.length,
            (i) => UnsplashImage.fromJSON(listOfImageMap[i])));
  });

  test('Should be able to search images', () async {
    expect(
        await galleryRepository.searchImages(query: "Code"),
        List.generate(listOfImageMap.length,
            (i) => UnsplashImage.fromJSON(listOfImageMap[i])));
  });
}
